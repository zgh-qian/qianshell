<template>
  <div
      class="qian-terminal-wrapper"
      :style="wrapperStyle"
      @click="handleClickWrapper"
  >
    <div ref="terminalRef" class="qian-terminal" :style="mainStyle">
      <a-collapse
...
import {useTerminalConfigStore} from "../command/commands/terminal/config/terminalConfigStore";
import useHint from "./hint";
import {LOCAL_USER} from "../command/commands/user/userConstant";
import CommandOutputType = QianTerminal.CommandOutputType;
import OutputType = QianTerminal.OutputType;
import CommandInputType = QianTerminal.CommandInputType;
import TerminalType = QianTerminal.TerminalType;
import TextOutputType = QianTerminal.TextOutputType;
import OutputStatusType = QianTerminal.OutputStatusType;
import UserType = User.UserType;

interface QianTerminalProps {
...
const props = withDefaults(defineProps<QianTerminalProps>(), {
  height: "400px",
  fullScreen: false,
...
onMounted(() => {
  ...
  if (welcomeTexts?.length > 0) {
    ...
    terminal.writeTextOutput(
        `Welcome to QianShell!`
    );
    terminal.writeTextOutput(
        `Author <a href="https://github.com/zgh-qian" target="_blank">qian</a>` +
        `: please input 'help' to begin`
    );
...
function handleClickWrapper(event: Event): void {
  //@ts-ignore
  if (event.target.className === "qian-terminal") {
    focusInput();
  }
}
...
.qian-terminal::-webkit-scrollbar {
  display: none;
}
...
.qian-terminal :deep(.ant-collapse) {
  background: none;
}

.qian-terminal :deep(.ant-collapse-borderless > .ant-collapse-item) {
  border: none;
}

.qian-terminal :deep(.ant-collapse-content > .ant-collapse-content-box) {
  padding: 0;
}
